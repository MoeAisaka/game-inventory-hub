...
import { quickGameWishlistSchema, quickUpdateGameWishlist } from "@/server/services/games";
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = quickGameWishlistSchema.safeParse(await safeJson(request));
  if (!parsed.success) return apiError("INVALID_REQUEST", "愿望单参数不合法", 400, id, z.flattenError(parsed.error).fieldErrors);
  try {
    const result = await quickUpdateGameWishlist(auth.userId, params.id, parsed.data, id);
    ...
    return apiOk({ game: result.game, active: parsed.data.active }, 200, id);
  } catch (error) {
    if (error instanceof Error && error.message === "GAME_WISHLIST_NOT_ELIGIBLE") {
      return apiError("INVALID_REQUEST", "仅未购买且没有游玩记录的游戏可加入愿望单", 400, id);
    }
    return apiError("INTERNAL_ERROR", "愿望单更新失败", 500, id);
  }
}
