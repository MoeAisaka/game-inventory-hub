...
import { ArrowRightLeft, ExternalLink, Pencil, Plus, Trash2, X } from "lucide-react";
import { useRouter } from "next/navigation";

type ItemView = {
  id: string; productName: string; color: string; brand: string | null; style: string | null; material: string | null;
  purchaseUrl: string | null; unitPrice: string | null; unopenedQuantity: number; openedQuantity: number; currentLocation: string | null; notes: string | null; version: number;
  [key: string]: unknown;
};
...
export function InventoryManager({ initialItems, total }: { initialItems: ItemView[]; total: number }) {
  ...
  const [mode, setMode] = useState<"new" | "edit" | "movement" | null>(null);
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api("/api/v1/inventory", { method: "POST", body: JSON.stringify({
        productName: String(form.get("productName")), purchaseUrl: String(form.get("purchaseUrl") || "") || null, color: String(form.get("color")), brand: String(form.get("brand") || "") || null,
  ...
  async function update(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      await api(`/api/v1/inventory/${selected.id}`, { method: "PATCH", body: JSON.stringify({
        productName: String(form.get("productName")), purchaseUrl: String(form.get("purchaseUrl") || "") || null,
        color: String(form.get("color")), brand: String(form.get("brand") || "") || null,
        style: String(form.get("style") || "") || null, material: String(form.get("material") || "") || null,
        unitPrice: form.get("unitPrice") === "" ? null : Number(form.get("unitPrice")), currentLocation: String(form.get("currentLocation") || "") || null,
        notes: String(form.get("notes") || "") || null, version: selected.version
      }) });
  ...
  return <>
    <section className="content-section">
      ...
      {initialItems.length ? <div className="table-wrap"><table><thead><tr><th>商品</th><th>购买链接</th><th>颜色/材质</th><th>未拆封</th><th>已拆封</th><th>总持有</th><th>位置</th><th>操作</th></tr></thead><tbody>{initialItems.map((item) => <tr key={item.id}><td><strong>{item.productName}</strong><small className="cell-note">{item.brand ?? "—"}</small></td><td>{item.purchaseUrl ? <a href={item.purchaseUrl} target="_blank" rel="noopener noreferrer" className="text-link">打开 <ExternalLink size={13} /></a> : "—"}</td><td>{item.color}<small className="cell-note">{item.material ?? item.style ?? "—"}</small></td><td>{item.unopenedQuantity}</td><td>{item.openedQuantity}</td><td><strong>{item.unopenedQuantity + item.openedQuantity}</strong></td><td>{item.currentLocation ?? "—"}</td><td><div className="row-actions"><button title="编辑" onClick={() => { setSelected(item); setMode("edit"); setMessage(""); }}><Pencil size={14} /></button><button title="调整库存" onClick={() => { setSelected(item); setMode("movement"); setMessage(""); }}><ArrowRightLeft size={14} /></button><button title="删除" disabled={busy} onClick={() => remove(item)}><Trash2 size={14} /></button></div></td></tr>)}</tbody></table></div> : <div className="empty-state">没有符合条件的库存记录。</div>}
    </section>
    {mode ? <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setMode(null); }}><div className="modal-panel narrow" role="dialog" aria-modal="true"><header><div><span className="eyebrow">{mode === "new" ? "NEW INVENTORY" : mode === "edit" ? "EDIT INVENTORY" : "STOCK MOVEMENT"}</span><h2>{mode === "new" ? "添加库存" : selected?.productName}</h2></div><button className="icon-button" onClick={() => setMode(null)}><X size={18} /></button></header>
      {mode === "new" ? <form onSubmit={create}><div className="form-grid"><label className="span-2">商品名称<input name="productName" required /></label><label className="span-2">购买链接<input name="purchaseUrl" type="url" placeholder="https://…（也可直接粘在商品名称末尾）" /></label><label>颜色<input name="color" required /></label><label>品牌<input name="brand" /></label><label>款式<input name="style" /></label><label>材质<input name="material" /></label><label>单价<input type="number" min="0" step="0.01" name="unitPrice" /></label><label>未拆封初值<input type="number" min="0" name="unopenedQuantity" defaultValue="0" /></label><label>已拆封初值<input type="number" min="0" name="openedQuantity" defaultValue="0" /></label><label>位置<input name="currentLocation" /></label><label className="span-2">备注<textarea name="notes" rows={3} /></label></div>{message ? <div className="inline-alert error">{message}</div> : null}<footer><button type="button" className="secondary-button" onClick={() => setMode(null)}>取消</button><button className="primary-button" disabled={busy}>保存</button></footer></form>
      : mode === "edit" && selected ? <form onSubmit={update}><div className="form-grid"><label className="span-2">商品名称<input name="productName" required defaultValue={selected.productName} /></label><label className="span-2">购买链接<input name="purchaseUrl" type="url" defaultValue={selected.purchaseUrl ?? ""} placeholder="https://…" /></label><label>颜色<input name="color" required defaultValue={selected.color} /></label><label>品牌<input name="brand" defaultValue={selected.brand ?? ""} /></label><label>款式<input name="style" defaultValue={selected.style ?? ""} /></label><label>材质<input name="material" defaultValue={selected.material ?? ""} /></label><label>单价<input type="number" min="0" step="0.01" name="unitPrice" defaultValue={selected.unitPrice ?? ""} /></label><label>位置<input name="currentLocation" defaultValue={selected.currentLocation ?? ""} /></label><label className="span-2">备注<textarea name="notes" rows={3} defaultValue={selected.notes ?? ""} /></label></div>{message ? <div className="inline-alert error">{message}</div> : null}<footer><button type="button" className="secondary-button" onClick={() => setMode(null)}>取消</button><button className="primary-button" disabled={busy}>保存修改</button></footer></form>
