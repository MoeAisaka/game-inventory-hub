...
import { externalAccounts, platformLibraryItems, platformWishlistItems, syncJobs } from "@/server/db/schema";
...
export default async function SyncPage() {
  ...
  const [accounts, jobs, library, platformCounts, wishlistCount] = await Promise.all([
    ...
    db.select({ value: count() }).from(platformWishlistItems).where(and(
      eq(platformWishlistItems.ownerUserId, session.userId),
  ...
  return <AppShell username={session.username} active="/sync">
    ...
    <SyncControl steamAccount={steam ? { steamId: steam.externalUserId, displayName: steam.displayName, status: steam.status, lastSyncedAt: steam.lastSyncedAt?.toISOString() ?? null } : null} steamReady={Boolean(config.STEAM_WEB_API_KEY)} igdbReady={Boolean(config.IGDB_CLIENT_ID && config.IGDB_CLIENT_SECRET)} wishlistCount={wishlistCount[0]?.value ?? 0} platformCounts={{ playstation: platformCounts.find((item) => item.provider === "PLAYSTATION")?.value ?? 0, nintendo: platformCounts.find((item) => item.provider === "NINTENDO")?.value ?? 0 }} />
    <SteamMatchReview
      summary={library.summary}
      counts={library.workbench.counts}
      items={library.workbench.items.map((item) => ({
        steamAppId: item.steamAppId,
        name: item.name,
        playtimeMinutes: item.playtimeMinutes,
        recentPlaytimeMinutes: item.recentPlaytimeMinutes,
        lastPlayedAt: item.lastPlayedAt instanceof Date ? item.lastPlayedAt.toISOString() : item.lastPlayedAt,
        iconUrl: item.iconUrl,
        matchMethod: item.matchMethod,
        licenseType: item.licenseType,
        lane: item.lane,
        hasPlayHistory: item.hasPlayHistory,
        suspectedNonGame: item.suspectedNonGame,
